    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            {Object.entries(marketingCosts).map(([key, value]) => (
              <div key={key} className="space-y-2">
                <Label htmlFor={key}>
                  Biaya {key.charAt(0).toUpperCase() + key.slice(1)}
                </Label>
                <Input
                  id={key}
                  type="number"
                  value={value}
                  onChange={(e) => handleMarketingCostChange(key as keyof typeof dummyMarketingCosts, e.target.value)}
                />
              </div>
            ))}
